...
class DocumentClassifierConfig:
    ...
    def __init__(self):
        ...
        self.text_files_folder = "extracted_text"  # Subfolder containing text files
        self.csv_file = "labels.csv"  # Should contain: label, category, example
...
class EmbeddingRetriever:
    ...
    def load_labels(self, csv_path: str) -> pd.DataFrame:
        ...
        required_columns = ['label', 'category', 'example']
    ...
    def generate_embeddings(self, force_regenerate: bool = False) -> Dict[str, np.ndarray]:
        ...
        # Prepare text for embedding (combine label, category, and example)
        embedding_texts = []
        label_keys = []
        ...
        for i in range(0, len(embedding_texts), batch_size):
            ...
            logger.info(f"Processing embedding batch {i//batch_size + 1}/{(len(embedding_texts) + batch_size - 1)//batch_size}")
        ...
        self.label_embeddings = {
            label: embedding 
            for label, embedding in zip(label_keys, all_embeddings)
        }
    ...
    def retrieve_similar_labels(self, query_predictions: List[str], top_k: int = 20) -> List[Tuple[str, float]]:
        ...
        filtered_labels = [
            (label, sim) for label, sim in sorted_labels 
...
class InferSignature(dspy.Signature):
    ...
    labels = dspy.OutputField(desc="Comma-separated list of predicted labels/keywords that best describe the document content")
...
class RankSignature(dspy.Signature):
    ...
    ranked_labels = dspy.OutputField(desc="Top relevant labels ranked by relevance, comma-separated")
...
class InferRetrieveRank(dspy.Module):
    ...
    def forward(self, text: str) -> dspy.Prediction:
        ...
        retrieved_labels = self.retriever.retrieve_similar_labels(
            initial_predictions, 
            top_k=self.config.max_retrieved_labels
        )
        
        if not retrieved_labels:
            logger.warning("No labels retrieved")
            return dspy.Prediction(labels=initial_predictions[:self.config.max_final_labels])
        ...
        rank_result = self.rank(
            text=text,
            candidate_labels=candidate_labels_text
        )
...
class DocumentClassifier:
    ...
    def classify_text(self, text: str) -> Dict[str, Any]:
        ...
        if not self.is_initialized:
            raise ValueError("Classifier not initialized. Call setup(), load_label_data(), and initialize_classifier() first")
    ...
    def classify_folder(self, folder_path: str = None, file_extension: str = ".txt") -> List[Dict[str, Any]]:
        """Classify all files in a folder"""
        
        if folder_path is None:
            # Use the extracted_text subfolder by default
            folder_path = os.path.join(self.config.data_folder, self.config.text_files_folder)
